...
var app = new Vue({
    ...
    methods: {
        cartProductsDisplay() {
            ...
            if (this.cart.length > 0) {
                this.cart.forEach(element => {
                    ...
                    if (
                        !result.includes(
                            this.cart.find(product => product.id === product_id)
                        )
                    ) {
                        result.push(element);
                    }
        ...
        removeProduct(product_id) {
            let index = this.cart.indexOf(
                this.cart.find(element => element.id === product_id)
            );
            if (
                this.cart.includes(
                    this.cart.find(element => element.id === product_id)
